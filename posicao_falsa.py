...
def posicao_falsa(funcao, limite_inicial, limite_final, precisao, iteracoes):
    ...
    while i <= iteracoes:
        point = (((limite_inicial*funcao(limite_final))-(limite_final*funcao(limite_inicial))) / (funcao(limite_final) - funcao(limite_inicial)))
        f_p = funcao(point)
        if math.fabs(f_p) <= precisao or (limite_final - limite_inicial) < precisao:
    ...
    print_result(((limite_inicial*funcao(limite_final) - limite_final*funcao(limite_inicial))/(funcao(limite_final) - funcao(limite_inicial))), iteracoes)
